...
class RecognizeFace():
    ...
    def __init__(self):
        ...
        # 处理存放所有人脸特征的 csv
        self.path_features_known_csv = "./feature/features2_all.csv"
        self.csv_rd = pd.read_csv(self.path_features_known_csv, header=None)
    ...
    def read_known_faces(self):
        '''读取已知人脸数据'''
        # print known faces
        for i in range(self.csv_rd.shape[0]):
            features_someone_arr = []
            for j in range(0, len(self.csv_rd.iloc[i,:])):
                features_someone_arr.append(self.csv_rd.iloc[i,:][j])
...
if __name__ == '__main__':
    RecognizeFace().recognize_for_sign()
